...
func extractHG3(fname, destDir string) bool {
	...
	// 添加新的目录
	outputDir := filepath.Join(prefix, basename[0:idx])
	// 确保目录存在
	...
	for {
		...
		var block []secRecord
		block, err = extractABlock(fs, uint32(len(blocks)), outputDir)
		if err != nil {
	...
	fsJson, err := os.Create(filepath.Join(prefix, "info.json"))
	if err != nil {
